...
contract('Team', accounts => {
    ...
    it("should have two members after adding two member", async () => {
        let team = await Team.deployed();
        let count = await team.countMembers.call();
        assert.equal(count, 1, "Initially should have one member");
        await team.addMember('kyungchan', 'QmVxxNFBjaGr2vyDWWvjn8wtfQB6X3jqYoVmrWxSFr5jpA');
        count = await team.countMembers.call();
        assert.equal(count, 2, "Initially should have zero member");
        let members = await team.getMembers.call();
        assert.equal(members.length, 2);
        assert.equal(members[0].name, 'cyoh');
        assert.equal(members[1].name, 'kyungchan');
    });
});
